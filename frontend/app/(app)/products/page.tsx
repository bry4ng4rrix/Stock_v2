...
export default function ProductsPage() {
  ...
  const [transferInitialCart, setTransferInitialCart] = useState<
    TransferCartItem[]
  >([]);
  ...
  const cartTotal = cartItems.reduce(
    (sum, item) => sum + Number(item.shell_price || 0) * (item.quantity ?? 1),
    0,
  );
  const checkoutDueInfo =
    !checkoutIsPaid && checkoutDueDate
      ? (() => {
  ...
  const statusLabel = (s: string) =>
    ({ in_stock: "En stock", low: "Faible", out_of_stock: "Rupture" })[s] ?? s;
    ...
    for (const p of products) {
      ...
      if (variants.length > 0) {
        ...
        for (const v of variants) {
          ...
          if (sizeKey)
            byColor.get(color)![sizeKey] =
              (byColor.get(color)![sizeKey] ?? 0) + v.quantity;
        }
    ...
    try {
      ...
      for (let idx = 0; idx < rows.length; idx++) {
        ...
        const alert_threshold =
          String(row["Seuil alerte"] ?? "5").trim() || "5";
        ...
        if (!name || !reference || !category || !shell_price) {
          errors.push(
            `Ligne ${line} : champs obligatoires manquants (Nom, Référence, Catégorie, Prix vente)`,
          );
          continue;
        }
      ...
      for (const [, prod] of productMap) {
        ...
        try {
          ...
          errors.push(
            `Ligne ${prod.line} (${prod.name}) : ${err.message || "Erreur API"}`,
          );
        }
      }
      ...
      if (importedCount > 0)
        toast.success(
          `${importedCount} produit(s) importé(s) avec QR codes générés`,
        );
  ...
  const handleAddProduct = async () => {
    if (!form.reference || !form.name || !form.category || !form.shell_price) {
      toast.error(
        "Veuillez remplir les champs obligatoires (Référence, Nom, Catégorie, Prix de vente)",
      );
      return;
    }
    if (!form.unit_price) {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      fd.append(
        "initial_quantity",
        String(editingProduct.initial_quantity ?? 0),
      );
      ...
      if (editingProduct.expiry_date)
        fd.append("expiry_date", editingProduct.expiry_date);
      fd.append("variants", JSON.stringify(editingProduct.variants ?? []));
      if (editingProduct.unit_price)
        fd.append("unit_price", String(editingProduct.unit_price));
      ...
      await djangoClient.patchFormData(
        `/users/products/${editingProduct.id}/`,
        fd,
      );
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          >
            <DialogContent className="max-w-sm">
              <DialogHeader>
                ...
                <DialogDescription>
                  Choisissez le magasin destination avant de sélectionner le
                  fichier Excel.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-3 py-2">
                ...
                <div className="space-y-2">
                  <Label>Magasin *</Label>
                  <Select
                    value={importMagasinId}
                    onValueChange={setImportMagasinId}
                  >
                    <SelectTrigger>
                      <SelectValue
                        placeholder={
                          storesLoading ? "Chargement..." : "Choisir un magasin"
                        }
                      />
                    </SelectTrigger>
                    <SelectContent>
                        ...
                        stores
                          .filter(
                            (s) =>
                              ...
                              (s.shop_name || "")
                                .toLowerCase()
                                .includes(importStoreFilter.toLowerCase()),
                          )
                          .map((s) => (
                            <SelectItem
                              key={s.magasin_id}
                              value={String(s.magasin_id)}
                            >
                              {s.shop_name}
                            </SelectItem>
                          ))
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button
                  onClick={confirmImportStore}
                  disabled={!importMagasinId}
                >
                  <Upload className="h-4 w-4 mr-2" />
                  Choisir le fichier
                </Button>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-orange-800 dark:text-orange-300">
                {expiringProducts.filter((p) => p.expiryInfo.days < 0).length >
                0
                  ? `${expiringProducts.filter((p) => p.expiryInfo.days < 0).length} produit(s) périmé(s) — `
                  : ""}
                {expiringProducts.filter((p) => p.expiryInfo.days >= 0).length}{" "}
                produit(s) expirant dans 30 jours
              </p>
              <ul className="mt-2 space-y-1">
                {expiringProducts.slice(0, 5).map((p) => (
                  ...
                  >
                    ...
                    <span
                      className={`shrink-0 text-xs font-semibold px-1.5 py-0.5 rounded ${p.expiryInfo.cls}`}
                    >
                      {p.expiryInfo.days < 0
                        ? "PÉRIMÉ"
                        : `expire le ${fmtDate(p.expiry_date)}`}
                    </span>
                  </li>
                ))}
              </ul>
        ...
        {isAdmin ? (
          ...
          user?.shop_name && (
            <div className="flex items-center rounded-md border bg-muted/50 px-3 py-2 text-sm whitespace-nowrap">
              Magasin :{" "}
              <span className="font-medium ml-1">{user.shop_name}</span>
            </div>
          )
        )}
                      ...
                      return (
                        ...
                        >
                            ...
                            {product.variants && product.variants.length > 0 ? (
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    setVariantsDialogProduct(product)
                                  }
                          ...
                          <TableCell className="text-right text-sm">
                            {Number(product.shell_price || 0).toLocaleString(
                              "fr-MG",
                            )}{" "}
                            Ar
                          </TableCell>
                          ...
                          {isAdmin && (
                            <TableCell className="text-right text-sm text-muted-foreground">
                              {product.unit_price != null
                                ? `${Number(product.unit_price).toLocaleString("fr-MG")} Ar`
                                : "—"}
                            </TableCell>
                          )}
                          <TableCell>
      ...
      <Dialog open={addStockDialogOpen} onOpenChange={setAddStockDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Ajoute une quantité positive au stock existant. Cette action ne
              peut qu'augmenter la quantité.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <div className="font-medium">
                {addingProduct?.name || ""} — Réf:{" "}
                {addingProduct?.reference || ""}
              </div>
              <div className="text-sm text-muted-foreground">
                Stock actuel: {addingProduct?.initial_quantity ?? 0}
              </div>
            </div>
            <div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                onClick={async () => {
                  ...
                  if (add <= 0) {
                    toast.error(
                      "Veuillez spécifier une quantité positive à ajouter.",
                    );
                    return;
                  }
                  setAddLoading(true);
                  try {
                    ...
                    await djangoClient.patchFormData(
                      `/users/products/${addingProduct.id}/`,
                      fd,
                    );
                    ...
                    toast.error(
                      err?.message || "Erreur lors de la mise à jour du stock",
                    );
                  } finally {
                    setAddLoading(false);
                  }
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-3 mt-3">
                      {[
                        {
                          label: "Principale",
                          url: imageUrl(previewProduct.image1),
                        },
                        {
                          label: "Secondaire 1",
                          url: imageUrl(previewProduct.image2),
                        },
                        {
                          label: "Secondaire 2",
                          url: imageUrl(previewProduct.image3),
                        },
                        {
                          label: "QR Code",
                          url: imageUrl(previewProduct.qr_code),
                        },
                      ]
                  ...
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                    <p>
                      <strong>Nom:</strong> {previewProduct.name}
                    </p>
                    <p>
                      <strong>Réf:</strong> {previewProduct.reference}
                    </p>
                    <p>
                      <strong>Catégorie:</strong> {previewProduct.category}
                    </p>
                    {previewProduct.magasin?.shop_name && (
                      <p>
              ...
              {cartItems.map((item) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      x{item.quantity} ·{" "}
                      {Number(item.shell_price || 0).toLocaleString("fr-MG")} Ar
                    </p>
                  </div>
                  <button
                    type="button"
          ...
          <Dialog open={checkoutOpen} onOpenChange={setCheckoutOpen}>
            <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Finalisez la vente de {cartCount} produit(s)
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleCheckout} className="space-y-4 pt-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setCheckoutPaymentAmount(e.target.value)
                        }
                        placeholder="0.00"
                      />
                    </div>
                ...
                {!checkoutIsPaid && checkoutDueInfo && (
                  <div
                    className={`rounded-xl p-3 text-sm ${checkoutDueInfo.isOverdue ? "bg-red-50 text-red-700 border border-red-200" : "bg-yellow-50 text-yellow-700 border border-yellow-200"}`}
                  >
                    {checkoutDueInfo.isOverdue ? (
                      <p>
                        Retard de paiement : {checkoutDueInfo.daysLate} jour(s).
                      </p>
                    ) : (
                      <p>
                        Échéance de paiement dans {checkoutDueInfo.daysUntil}{" "}
                        jour(s).
                      </p>
                    )}
                    <p className="text-xs text-muted-foreground">
                      Échéance : {checkoutDueDate}
                    </p>
                  </div>
                )}
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open) setVariantsDialogProduct(null);
        }}
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {variantsDialogProduct?.variants?.length ?? 0} variante(s) · stock
              total : {variantsDialogProduct?.initial_quantity ?? 0}
            </DialogDescription>
          </DialogHeader>
          {variantsDialogProduct && (
            <VariantsDialogBody
              variants={variantsDialogProduct.variants ?? []}
            />
          )}
        </DialogContent>
      </Dialog>
...
function VariantsDialogBody({ variants }: { variants: ProductVariant[] }) {
  ...
  const displayed = activeColor
    ? { [activeColor]: colorGroups[activeColor] }
    : colorGroups;
        ...
        {Object.entries(displayed).map(([color, vars]) => (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-muted-foreground">
                {vars.reduce((s, v) => s + v.quantity, 0)} unités
              </span>
            </div>
            <div className="flex flex-wrap gap-2">
...
function ProductForm({
  ...
  const totalVariantQty = variants.reduce(
    (s: number, v: ProductVariant) => s + (v.quantity || 0),
    0,
  );
  ...
  const addVariant = () => {
    ...
    const updated = [
      ...variants,
      { size: variantSize.trim(), color: variantColor.trim(), quantity: qty },
    ];
    set("variants", updated);
    set(
      "initial_quantity",
      String(updated.reduce((s, v) => s + v.quantity, 0)),
    );
    setVariantSize("");
    setVariantColor("");
    setVariantQty("");
  ...
  const handleGenerateQr = async () => {
    ...
    if (!qrSource) {
      toast.error(
        "Veuillez renseigner la référence ou le nom du produit pour générer un QR code.",
      );
      return;
    }
    try {
      const dataUrl = await generateSimpleQRCode(qrSource);
      const qrFile = dataURLtoFile(
        dataUrl,
        `qr-${qrSource.replace(/\s+/g, "-")}.png`,
      );
      set("qr_code", qrFile);
  ...
  const filteredStores = stores.filter((store) => {
    const label = String(
      store.shop_name || store.manager?.full_name || "",
    ).toLowerCase();
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <SelectContent>
                {storesLoading ? (
                  ...
                  filteredStores.map((store) => (
                    <SelectItem
                      key={store.magasin_id || store.id || store.shop_name}
                      value={String(store.magasin_id || store.id || "")}
                    >
                      {store.shop_name ||
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            step="0.01"
            placeholder="0.00"
            value={form.shell_price || ""}
            onChange={(e) => set("shell_price", e.target.value)}
          />
        </div>
      ...
      <div className="space-y-3 rounded-lg border border-border bg-muted/20 dark:bg-muted/10 p-4">
        ...
        <div className="flex flex-wrap gap-2 items-end">
          <div className="space-y-1 min-w-[90px]">
            ...
            <Select
              value={variantSize || "__none__"}
              onValueChange={(v) => setVariantSize(v === "__none__" ? "" : v)}
            >
              <SelectTrigger>
          ...
          <Button
            ...
            onClick={addVariant}
            disabled={
              Number(variantQty) <= 0 || (!variantSize && !variantColor)
            }
            className="shrink-0"
          >
        ...
        {variants.length > 0 ? (
            ...
            {variants.map((v, idx) => (
              ...
              >
                <input
                  ...
                  value={v.quantity}
                  onChange={(e) =>
                    updateVariantQty(idx, Number(e.target.value) || 0)
                  }
          ...
          <p className="text-xs text-muted-foreground italic">
            Aucune variante ajoutée. Vous pouvez aussi laisser vide et utiliser
            la quantité directe ci-dessous.
          </p>
        )}
