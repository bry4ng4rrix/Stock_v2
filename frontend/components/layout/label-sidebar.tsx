...
export function LabelSidebar() {
  ...
  const fetchCounts = useCallback(async () => {
    try {
      const [requests, expiring] = await Promise.all([
        djangoClient.platformAdmin.listRequests('pending'),
        djangoClient.platformAdmin.getExpiringSoon(),
      ]);
  ...
  useEffect(() => {
    fetchCounts();
    const interval = setInterval(fetchCounts, COUNT_POLL_MS);
          ...
          <div className="p-6 border-b border-slate-200 bg-linear-to-r from-slate-50/50 to-transparent dark:border-slate-700/50 dark:from-slate-900/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium dark:text-slate-400">Plateforme</p>
              </div>
            </div>
          </div>
